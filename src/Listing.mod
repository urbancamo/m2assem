...
BEGIN
  ...
  EndTime :=	((LONGCARD(EndHours) * 3600) +
  		(LONGCARD(EndMinutes) * 60) +
  		(LONGCARD(EndSeconds)));
...
BEGIN
  ErrorPending := TRUE;
  MakeSubstring(ErrorMessage, 1, LengthOfString(ErrorMessage), ErrorString)
END AddError;
...
BEGIN
  ...
  StartTime :=	(LONGCARD(StartHours) * 3600) +
  		(LONGCARD(StartMinutes) * 60) +
  		(LONGCARD(StartSeconds));
