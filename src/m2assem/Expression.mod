...
VAR
  ...
  VAR
	...
	BEGIN
	  CASE In OF
	    '0'..'9': RETURN LONGINT(ORD(In) - ORD('0'))      |
	    'a'..'f': RETURN LONGINT(ORD(In) - ORD('a')) + 10 |
	    'A'..'F': RETURN LONGINT(ORD(In) - ORD('A')) + 10
	  ELSE
  ...
  PROCEDURE Eval(Operand1: LONGINT; Operand1Status: SymbolStatus;
    ...
    PROCEDURE DoShr(Op1, Op2: LONGINT): LONGINT; FORWARD;
