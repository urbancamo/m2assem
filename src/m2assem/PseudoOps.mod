      ...
      IF DecInstr.NoOfOperands = 1 THEN
        IF DecInstr.Operand[1].NoOfIndices = 0 THEN
          ...
          CASE DecInstr.Modifier OF
            'B': Length := 1;
                 IF PassNo = 2 THEN
                   Data[1] := Word(SHORTINT(DATA));
                   AddCode(CurrentLocation(), Data, Length)
                 END
          |
            'W': Length := 2;
                 IF PassNo = 2 THEN
                   Data[1] := Word(INTEGER(DATA));
                   AddCode(CurrentLocation(), Data, Length)
                 END
          |
            'L': Length := 4;
  ...
  BEGIN
    ...
    IF DecInstr.NoOfOperands = 1 THEN
      IF DecInstr.Operand[1].NoOfIndices = 0 THEN
        ...
        SetLocation(CurrentLocation() + LONGCARD(RESSize))
      ELSE
        Raise(Code, Error, IndicesCountError)
      END
    ELSE
